            ...
            <aside className="fixed bottom-4 left-1/2 -translate-x-1/2 md:static md:translate-x-0 w-[94%] max-w-[360px] md:w-24 bg-[#0F1118]/80 backdrop-blur-3xl border border-white/10 md:border-none md:bg-transparent rounded-[2rem] md:rounded-none flex md:flex-col items-center py-2.5 px-6 md:py-12 gap-4 md:gap-12 justify-center z-50 shadow-2xl shadow-black/80 md:shadow-none">
                <div className="flex md:flex-col gap-8 md:gap-14 items-center w-full md:w-auto justify-around md:justify-start">
                    ...
                    <nav className="flex md:flex-col gap-10 md:gap-12 w-full md:w-auto justify-around md:justify-center">
                        <button onClick={() => setActiveTab('studio')} className={`flex flex-col items-center gap-1.5 group transition-all duration-300 ${activeTab === 'studio' ? 'text-blue-500' : 'text-slate-600 hover:text-slate-400'}`}>
                            <div className={`p-3.5 rounded-[1.2rem] transition-all duration-300 relative ${activeTab === 'studio' ? 'bg-blue-600 text-white shadow-xl shadow-blue-600/40' : 'bg-white/5 text-slate-500 md:bg-transparent md:hover:bg-white/5'}`}>
                                <Layout className="w-5 h-5" />
                            </div>
                            <span className={`text-[9px] font-black uppercase tracking-[0.2em] transition-all duration-300 ${activeTab === 'studio' ? 'opacity-100' : 'opacity-40'}`}>Studio</span>
                        </button>
                        <button onClick={() => setActiveTab('vault')} className={`flex flex-col items-center gap-1.5 group transition-all duration-300 ${activeTab === 'vault' ? 'text-blue-500' : 'text-slate-600 hover:text-slate-400'}`}>
                            <div className={`p-3.5 rounded-[1.2rem] transition-all duration-300 relative ${activeTab === 'vault' ? 'bg-blue-600 text-white shadow-xl shadow-blue-600/40' : 'bg-white/5 text-slate-500 md:bg-transparent md:hover:bg-white/5'}`}>
                                <HistoryIcon className="w-5 h-5" />
                            </div>
                            <span className={`text-[9px] font-black uppercase tracking-[0.2em] transition-all duration-300 ${activeTab === 'vault' ? 'opacity-100' : 'opacity-40'}`}>Vault</span>
                        </button>
                        <button onClick={() => setActiveTab('profile')} className={`flex flex-col items-center gap-1.5 group transition-all duration-300 ${activeTab === 'profile' ? 'text-blue-500' : 'text-slate-600 hover:text-slate-400'}`}>
                            <div className={`p-3.5 rounded-[1.2rem] transition-all duration-300 relative ${activeTab === 'profile' ? 'bg-blue-600 text-white shadow-xl shadow-blue-600/40' : 'bg-white/5 text-slate-500 md:bg-transparent md:hover:bg-white/5'}`}>
                                <User className="w-5 h-5" />
                            </div>
                            <span className={`text-[9px] font-black uppercase tracking-[0.2em] transition-all duration-300 ${activeTab === 'profile' ? 'opacity-100' : 'opacity-40'}`}>Profile</span>
                        </button>
                    </nav>
                </div>
            </aside>
                    ...
                    <div className="flex flex-col items-center max-w-2xl mx-auto w-full animate-in fade-in slide-in-from-bottom-8 duration-700">
                        ...
                        <div className="w-full grid gap-4">
                            <div className="bg-[#0F1118] border border-white/5 p-8 rounded-[2.5rem] flex flex-col sm:flex-row items-center justify-between gap-6 shadow-xl relative overflow-hidden group hover:border-blue-500/30 transition-all duration-500">
                                ...
                                {!hasPlan && (
                                    <button onClick={() => setShowPlans(true)} className="relative w-full sm:w-auto px-8 py-4 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-xl shadow-blue-600/20 transition-all active:scale-95 flex items-center justify-center gap-3 group">
                            ...
                            <div className="bg-[#0F1118]/50 border border-white/5 p-8 rounded-[2.5rem] flex items-center justify-between shadow-xl">
                                <div className="flex items-center gap-6">
                                    ...
                                    <div className="text-left">
                                        ...
                                        <div className="text-xl font-black text-white tracking-widest">{vaultItems.length} Master Tracks</div>
                                    </div>
                                </div>
                            </div>
